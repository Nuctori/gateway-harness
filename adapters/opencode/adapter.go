...
const (
	HookPromptAppend      = "tui.prompt.append"
...
func (Adapter) Capability() gharness.Capability {
	return gharness.Capability{
		SourceHooks: []string{HookPromptAppend, HookSessionCompacting},
...
func buildEvent(input gharness.AdapterInput, contextOnly bool) (event.Event, error) {
	...
	if payload.Hook == HookPromptAppend {
		if contextOnly {
	...
	if payload.Hook == HookSessionCompacting {
	...
	return event.Event{}, fmt.Errorf("unsupported opencode hook %q", payload.Hook)
}
...
func hookInput(payload any) (HookInput, error) {
	...
	case *HookInput:
		if v == nil {
			return HookInput{}, fmt.Errorf("payload is nil *opencode.HookInput")
		}
		return *v, nil
	default:
		return HookInput{}, fmt.Errorf("payload is not opencode.HookInput (got %T)", payload)
	}
}
